...
function switchPlan(type) {
    // Get Elements
    const contentWeek = document.getElementById('plans-week');
    const contentSat = document.getElementById('plans-sat');
    const btns = document.querySelectorAll('.plans-toggle .toggle-btn'); // More specific selector

    if (!contentWeek || !contentSat) return;
    ...
    if (type === 'week') {
        ...
        contentSat.style.display = 'none';
        // First button is likely Week, but let's be safe. 
        // In the HTML: <button ... onclick="switchPlan('week')"> is first
        btns[0].classList.add('active');
    } else {
        contentWeek.style.display = 'none';
        contentSat.style.display = 'grid';
